...
public class OptionServiceImpl implements OptionService {
    ...
    private static final String CACHE_PREFIX = "option:";
    ...
    @Override
    @SimpleServiceLog("获取加载到前端的参数的checksum")
    public String getFrontLoadOptionsChecksum() {
        return (String) redis.opsForValue().get("optionsChecksum");
    }

    @Override
    @SimpleServiceLog("更新加载到后端的参数缓存")
    public void updateBackLoadOptionCache() {
        List<Option> options = optionRepository.findByActiveAndBackLoad(true, true);
        if (options != null) {
            for (Option option : options) {
